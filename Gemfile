...
gem 'activerecord-nulldb-adapter'      , '>= 0.5.0' # Running Rake tasks at build time before DB is set up. TODO: Still needed?
...
gem 'devise', '~> 4.8', '>= 4.8.0' # Authentication framework
gem 'devise-jwt', '0.8.1' # JWT authentication for native Primero users
gem 'faraday',             '~> 0.17'   # Ruby HTTP client
gem 'file_validators', '~> 3.0', '>= 3.0.0' # ActiveRecord extension for validating attachment file sizes
gem 'i18n-js',             '~> 3.4'    # Shares Rails i18n strings with the front end
gem 'jbuilder', '~> 2.9', '>= 2.9.0' # JSON templating for the API
gem 'mini_magick',         '~> 4.9.4'  # Ruby bindings for ImageMagick, resize attachments. TODO: Is this necessary?
gem 'minipack', '~> 0.3', '>= 0.3.4' # An alternative to Webpacker. TODO: Is this still needed? In prod?
...
gem 'rack',                '>= 2.2.0'
gem 'rack-attack', '>= 6.7.0' # Rack middleware to rate limit sensetive routes, such as those used for auth
gem 'rails', '7.1.0'
gem 'rake',                '~> 12.3'
...
gem 'twitter_cldr', '~> 4.4', '>= 4.4.5' # Localization for dates, money. TODO: Is this still used?
gem 'tzinfo',              '~> 1.2'    # Security assertion on implicit dependency.
gem 'tzinfo-data', '~> 1.2020', '>= 1.2020.1' # Security assertion on implicit dependency.
...
group :development, :test do
  ...
  gem 'ci_reporter',                '~> 2.0'
  gem 'factory_bot', '~> 5.1', '>= 5.1.0'
  gem 'foreman'
  gem 'i18n-tasks', '~> 0.9', '>= 0.9.30'
  gem 'json_spec',                  '~> 1.1'
